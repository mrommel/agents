...
class Village: GameObject {
	...
	override func execute(action: GameObjectAction) {
		...
		if action == Village.openVillage {
			...
			self.executeImplementField(from: action)
			...
			if let selectedAction = action as? GameObjectActionWithPoint, let tile = self.engine?.scene.map.tile(at: selectedAction.point!), let point = selectedAction.point {
	...
	func executeImplementField(from action: GameObjectAction) {
		if let selectedAction = action as? GameObjectActionWithPoint {
			// add to map
